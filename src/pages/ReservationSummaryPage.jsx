...
function ReservationSummarySkeleton() {
  return (
    <Stack spacing={2.5}>
...
export default function ReservationSummaryPage() {
  ...
  }, [id, navigate, patientId, showError]);
  ...
  const handleSubmitPrescription = async () => {
    ...
    const payload = {
      ...
      status: statusValue || 'arrived',
      is_examination: prescriptionType === 'examination',
      discount: Number(discountNumber.toFixed(2)),
      medicines,
    };
    setSavingPrescription(true);
    try {
      await post(`/reservations/${encodeURIComponent(id)}/prescription`, payload);
