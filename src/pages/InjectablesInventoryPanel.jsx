...
/**
 * Injectables tab: products API (`/products`).
 */
export default function InjectablesInventoryPanel({ onListCountChange } = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const query = buildProductsListQuery(page, rowsPerPage, appliedName.trim(), appliedServiceId);
        const data = await get(`/products?${query}`);
        if (cancelled) return;
        const parsed = parsePaginatedList(data, { listKeys: ['products'] });
        setRows(parsed.rows);
        ...
        if (!cancelled) {
          const msg =
            ...
            err?.message ||
            'Could not load injectables.';
          showError(typeof msg === 'string' ? msg : 'Could not load injectables.');
          setRows([]);
          setTotalCount(0);
  ...
  const requestDelete = useCallback(
    row => {
      ...
      setDeleteTarget({
        id: productId,
        name: row.name?.trim() || `Injectable #${productId}`,
      });
    },
    [showInfo]
  );
  ...
  const columns = useMemo(
    () => [
      ...
      { id: 'quantity', label: 'Qty', minWidth: 70 },
      { id: 'volume', label: 'Volume', minWidth: 90 },
    ...
    <>
      ...
      >
        <TextField
          ...
          aria-label="Search injectables by name"
          slotProps={{
            input: {
              startAdornment: (
        ...
        <FormControl size="small" sx={{ minWidth: { xs: '100%', md: 220 } }}>
          <InputLabel id="injectables-filter-service-label">Service</InputLabel>
          <Select
            labelId="injectables-filter-service-label"
            label="Service"
            value={serviceInput}
      ...
      <PaginatedTable
        ...
        rows={paginatedRows}
        loading={loading}
        emptyMessage="No injectables yet."
        page={page}
      ...
      <Dialog
        ...
        aria-labelledby="delete-injectable-dialog-title"
      >
        <DialogTitle id="delete-injectable-dialog-title">Delete injectable?</DialogTitle>
        <DialogContent>
